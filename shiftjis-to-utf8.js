...
(function(exports) {
  ...
  var decodeLead=that.decodeLead=[];
  var decodeTable=that.decodeTable=[];
  var seq=that.seq=[];
  var decodeTableSeq=that.decodeTableSeq=[];
  ...
  for (var j = 0; j < table.length; j++) { // Chunks.
      ...
      if (curAddr < 0x100) {
          ...
          if (decodeLead[curAddr >> 8] >= 0)
              throw new Error("Overwrite lead byte in table " + i + " in shiftjis at chunk " + chunk[0]);
          
          decodeLead[curAddr >> 8] = -2; // DBCS lead byte.
          writeTable = decodeTable;
          curAddr -= 0x8000;
          if (curAddr < 0)
              throw new Error("DB address < 0x8000 in table " + i + " in shiftjis at chunk " + chunk[0]);
      }
      else
          throw new Error("Unsupported address in table " + i + " in shiftjis at chunk " + chunk[0]);
      ...
      for (var k = 1; k < chunk.length; k++) {
          ...
          if (typeof part === "string") { // String, write as-is.
              for (var l = 0; l < part.length;) {
                  ...
                  if (0xD800 <= code && code < 0xDC00) { // Surrogate
                      ...
                      if (0xDC00 <= codeTrail && codeTrail < 0xE000)
                          ...
                          throw new Error("Incorrect surrogate pair in table " + i + " in shiftjis at chunk " + chunk[0]);
                  }
              ...
              throw new Error("Incorrect value type '" + typeof part + "' in table " + i + " in shiftjis at chunk " + chunk[0]);
      }
  }

  function decoderDBCSWrite(buf) {
      var newBuf = new Array(buf.length*2),
          leadBytes = that.leadBytes, uCode;
      
      for (var i = 0, j = 0; i < buf.length; i++) {
          var curByte = buf[i];
          if (leadBytes === -1) { // We have no leading byte in buffer.
              uCode = that.decodeLead[curByte];
              ...
              uCode = that.decodeTable[(leadBytes << 8) + curByte - 0x8000];
          ...
          if (uCode === -1) { // Undefined char.
              ...
              var seq = that.decodeTableSeq[-uCode];
      ...
      that.leadBytes = leadBytes;

      //return newBuf.slice(0, j).toString('ucs2');

      var rv = ""; for(var i =0; i<j; i+=2) { rv += String.fromCharCode(newBuf[i] | newBuf[i+1]*256); };
      return rv;
  }
